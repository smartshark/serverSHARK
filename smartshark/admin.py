...
class CommitVerificationAdmin(admin.ModelAdmin):
    ...
    def delete_ces_list(self, request, queryset):
        ...
        if request.POST.get('post'):
            ...
            del_list_count, changed_commit_id_count, childs = handler.clear_code_entity_state_lists(revisions, queryset[0].vcs_system)
            logger.info('Deleted code_entity_states list for {} commits, changed commit_id on {} code entity states for {} childs'.format(del_list_count, changed_commit_id_count, childs))

            messages.info(request, 'Deleted code_entity_states list for {} commits, changed commit_id on {} code entity states for {} childs'.format(del_list_count, changed_commit_id_count, childs))
